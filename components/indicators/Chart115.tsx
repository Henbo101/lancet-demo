...
type GeoLevel = 'Global' | 'WHO Region' | 'LC Region' | 'HDI Group';
type ValueType = 'AN' | 'AF';
...
export default function Chart115() {
  ...
  const [valueType, setValueType] = useState<ValueType>('AN');
  ...
  const lines: LineData[] = useMemo(() => {
    ...
    const groups = [...new Set(hdiData.map((d) => d['HDI Group']))];
    return groups.map((g) => ({
      name: g,
      points: hdiData
        .filter((d) => d['HDI Group'] === g)
        .map((d) => ({ year: d.Year, value: d[vk] })),
    }));
  }, [geoLevel, valueType]);
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 py-4 mb-4 border-b border-outline-variant/30">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="HDI Group">HDI Group</option>
          </select>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {(['AN', 'AF'] as const).map((v) => (
            ...
            >
              {v === 'AN' ? 'Deaths (AN)' : 'Attributable Fraction % (AF)'}
            </button>
          ))}
        </div>
      </div>
              ...
              <svg width={width} height={height}>
                <Group left={margin.left} top={margin.top}>
                  ...
                  <AxisLeft
                    ...
                    label={valueType === 'AN' ? 'Deaths' : 'Attributable Fraction (%)'}
                    labelProps={{
                      fill: '#40484e',
                      fontSize: 12,
                      fontFamily: "'Open Sans', sans-serif",
